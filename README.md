...
- **Units from the Module**: MQTT carries bare values, so for metrics this integration has no definition for it asks the module (`metrics list -n`) which unit each metric is in, instead of guessing from the topic name. New metrics from a firmware update or an unsupported vehicle get the right unit without an integration update. What the module answered is remembered, so the units stay the same when Home Assistant restarts while the car is away
...
- **Vehicle-Specific Metrics**: Special support for VW e-UP!, Smart ForTwo (453/EQ), Smart ED (451), Nissan Leaf, Renault Twizy, and MG ZS-EV - with additional vehicle models planned
...
A: Yes, the integration is vehicle-agnostic and works with any vehicle supported by OVMS. Vehicle-specific enhancements are provided for some models like VW e-UP!, Smart ForTwo (453/EQ), Smart ED (451), Nissan Leaf, Renault Twizy and MG ZS-EV.
