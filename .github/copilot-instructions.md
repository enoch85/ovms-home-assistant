...
**Problem**: Home Assistant stores the *displayed* state. Adopting it as `native_value` double-converts for users with a different display unit (°F, mi), and a stored text state makes Home Assistant reject a sensor that is now numeric ("Error adding entity").
**Solution**: `OVMSSensor._can_restore_state` - the live MQTT payload always wins; the stored state only fills in when there is no live value, and for a numeric sensor only when it is a number in the sensor's own native unit.
