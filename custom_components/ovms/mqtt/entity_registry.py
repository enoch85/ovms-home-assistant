...
class EntityRegistry:
    ...
    Supports multiple entities per topic (e.g., a sensor and a switch for the same metric).
    """

    def __init__(self):
        """Initialize the entity registry."""
        # Maps topic -> list of entity_ids (supports multiple entities per topic)
        ...
        # Maps (topic, entity_id) -> priority for multi-entity support
        self.priorities: Dict[tuple, int] = {}
    ...
    def register_entity(
        ...
        try:
            ...
            if entity_id in self.topics[topic]:
                _LOGGER.debug(
                    "Entity %s already registered for topic %s", entity_id, topic
                )
                return False
            ...
            _LOGGER.debug(
                "Registered %s (%s) for topic %s (priority %d)",
                entity_id,
                entity_type,
                topic,
                priority,
    ...
    def get_entity_for_topic(self, topic: str) -> Optional[str]:
        """Get the primary entity ID associated with a topic.

        Returns the first entity if multiple are registered (for backward compatibility).

        Args:
            topic: The MQTT topic to look up

        Returns:
            The primary entity ID, or None if no entity registered for this topic
        """
    ...
    def get_entities_for_topic(self, topic: str) -> List[str]:
        ...
        Returns:
            List of entity IDs registered for this topic (may be empty)
        """
