...
class EntityStalenessManager:
    ...
    # Legacy compatibility methods - no longer used but keep for compatibility
    def track_entity_update(self, entity_id: str) -> None:
        """Legacy method - no longer needed as we use HA's availability."""
        pass
    ...
    def is_entity_stale(self, entity_id: str) -> bool:
        """Legacy method - check if entity is unavailable in HA."""
        if not self._enabled:
            return False
    ...
    def get_entity_age_hours(self, entity_id: str) -> Optional[float]:
        """Get how long an entity has been in its current state."""
        state = self.hass.states.get(entity_id)
        if state and hasattr(state, 'last_updated') and state.last_updated:
            return (self.hass.loop.time() - state.last_updated.timestamp()) / 3600
        return None

    def get_stale_entities(self):
        """Legacy method - get unavailable entities."""
        entity_registry = er.async_get(self.hass)
        unavailable = []
    ...
    def remove_entity(self, entity_id: str) -> None:
        """Legacy method - no longer tracks entities individually."""
        pass
