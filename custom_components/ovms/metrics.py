...
from homeassistant.const import PERCENTAGE, UnitOfElectricCurrent, \
    UnitOfElectricPotential, UnitOfEnergy, UnitOfLength, UnitOfPower, \
    UnitOfPressure, UnitOfSpeed, UnitOfTemperature, UnitOfTime
...
METRIC_DEFINITIONS = {  # System metrics
                        # 12V Battery metrics
    ...
    'm.egpio.input': {
        ...
        'description': 'EGPIO input port state (ports 0\xe2\x80\xa69, present=high)',
        'icon': 'mdi:gpio',
        'category': CATEGORY_SYSTEM,
        'entity_category': EntityCategory.DIAGNOSTIC,
        },
    ...
    'v.g.type': {
        ...
        'description': 'Connection type (chademo, ccs, \xe2\x80\xa6)',
        'icon': 'mdi:ev-plug-type2',
        'category': CATEGORY_POWER,
        },
    'v.g.voltage': {
...
TOPIC_PATTERNS = {
    ...
    'status': {'name': 'Status', 'icon': 'mdi:information-outline',
               'category': CATEGORY_SYSTEM},
    'alert': {
        'name': 'Alert',
        'icon': 'mdi:alert',
    ...
    'xvu': {'name': 'VW eUP!', 'icon': 'mdi:car-electric',
            'category': CATEGORY_VW_EUP},
    }
...
METRIC_CATEGORIES = {
    CATEGORY_BATTERY: [k for (k, v) in METRIC_DEFINITIONS.items()
                       if v.get('category') == CATEGORY_BATTERY],
    CATEGORY_CHARGING: [k for (k, v) in METRIC_DEFINITIONS.items()
                        if v.get('category') == CATEGORY_CHARGING],
    CATEGORY_CLIMATE: [k for (k, v) in METRIC_DEFINITIONS.items()
                       if v.get('category') == CATEGORY_CLIMATE],
    CATEGORY_DOOR: [k for (k, v) in METRIC_DEFINITIONS.items()
                    if v.get('category') == CATEGORY_DOOR],
    CATEGORY_LOCATION: [k for (k, v) in METRIC_DEFINITIONS.items()
                        if v.get('category') == CATEGORY_LOCATION],
    CATEGORY_MOTOR: [k for (k, v) in METRIC_DEFINITIONS.items()
                     if v.get('category') == CATEGORY_MOTOR],
    CATEGORY_TRIP: [k for (k, v) in METRIC_DEFINITIONS.items()
                    if v.get('category') == CATEGORY_TRIP],
    CATEGORY_DEVICE: [k for (k, v) in METRIC_DEFINITIONS.items()
                      if v.get('category') == CATEGORY_DEVICE],
    CATEGORY_DIAGNOSTIC: [k for (k, v) in METRIC_DEFINITIONS.items()
                          if v.get('category') == CATEGORY_DIAGNOSTIC],
    CATEGORY_POWER: [k for (k, v) in METRIC_DEFINITIONS.items()
                     if v.get('category') == CATEGORY_POWER],
    CATEGORY_NETWORK: [k for (k, v) in METRIC_DEFINITIONS.items()
                       if v.get('category') == CATEGORY_NETWORK],
    CATEGORY_SYSTEM: [k for (k, v) in METRIC_DEFINITIONS.items()
                      if v.get('category') == CATEGORY_SYSTEM],
    CATEGORY_TIRE: [k for (k, v) in METRIC_DEFINITIONS.items()
                    if v.get('category') == CATEGORY_TIRE],
    CATEGORY_VW_EUP: [k for (k, v) in METRIC_DEFINITIONS.items()
                      if v.get('category') == CATEGORY_VW_EUP],
    }

# Binary metrics that should be boolean

BINARY_METRICS = [k for (k, v) in METRIC_DEFINITIONS.items()
