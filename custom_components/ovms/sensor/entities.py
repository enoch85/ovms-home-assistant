...
from .factory import (
    ...
    add_device_specific_attributes,
    create_cell_sensors,
)
...
class CellVoltageSensor(SensorEntity, RestoreEntity):
    ...
    def __init__(
        ...
        if self._attr_device_class in (
            ...
            self._attr_extra_state_attributes["original_device_class"] = (
            ...
            if self._attr_device_class == SensorDeviceClass.TIMESTAMP:
                ...
                self._attr_extra_state_attributes["original_state_class"] = (
                    self._attr_state_class
                )
                self._attr_extra_state_attributes["original_unit"] = (
        ...
        # Parse the value
        if requires_numeric_value(
            self._attr_device_class, self._attr_state_class
        ) and is_special_state_value(initial_state):
            self._parsed_value = None
        else:
            try:
                if isinstance(initial_state, (int, float)) or (
                    isinstance(initial_state, str)
                    and initial_state.replace(".", "", 1).isdigit()
                ):
        ...
        device_class_for_formatting = (
            self._attr_extra_state_attributes.get("original_device_class")
    ...
    async def async_added_to_hass(self) -> None:
                    ...
                    if state.attributes and "raw_value" in state.attributes:
                        ...
                        if raw_value is not None:
                            ...
                            self._attr_native_value = (
                                state.state
                            )  # Keep the formatted string
                            self._attr_extra_state_attributes["raw_value"] = raw_value
                            self._attr_extra_state_attributes["formatted_short"] = (
                                state.state
                            )
        ...
        def update_state(payload: Any) -> None:
            ...
            if requires_numeric_value(
                self._attr_device_class, self._attr_state_class
            ) and is_special_state_value(payload):
                self._parsed_value = None
            else:
                try:
                    if isinstance(payload, (int, float)) or (
                        isinstance(payload, str)
                        and payload.replace(".", "", 1).isdigit()
                    ):
...
class OVMSSensor(SensorEntity, RestoreEntity):
    ...
    def __init__(
        ...
        self._is_cell_sensor = (
            (
                (
                    "cell" in self._topic.lower()
                    or "voltage" in self._topic.lower()
                    or "temp" in self._topic.lower()
                )
                and self._attr_extra_state_attributes.get("category") == "battery"
            )
            or self._attr_extra_state_attributes.get("has_cell_data", False)
            or self._is_tire_sensor()  # All tire metrics have multiple values
        )
        ...
        if self._is_tire_sensor():
            ...
            if (
                "pressure" in self._topic.lower()
                or "emgcy" in self._topic.lower()
                or "diff" in self._topic.lower()
            ):
    ...
    def _handle_cell_values(self, payload: str) -> None:
        ...
        try:
            ...
            for old_key in ["cell_values", "values", "cell_count"]:
                if old_key in self._attr_extra_state_attributes and (
                    old_key != "cell_values" or self._stat_type != "cell"
                ):
            ...
            for key in self._attr_extra_state_attributes.keys():
                ...
                # Also remove standalone "Cell X" or "Pressure X" attributes
                    ...
                    key.startswith("Cell ")
                    or key.startswith("Pressure ")
                    or key.startswith("Temperature ")
                ):
                    keys_to_remove.append(key)
            ...
            for i, val in enumerate(values):
                if self._is_tire_sensor() and i < 4:
                    ...
                    self._attr_extra_state_attributes[
                        f"{self._stat_type}_{position_code}"
                    ] = val
                else:
            ...
            if (
                hasattr(self, "_cell_sensors_created")
                and self._cell_sensors_created
                and CREATE_INDIVIDUAL_CELL_SENSORS
            ):
    ...
    def _create_cell_sensors(self, cell_values: List[float]) -> None:
        ...
        sensor_configs = create_cell_sensors(
            ...
            {
                "name": self.name,
                "category": self._attr_extra_state_attributes.get(
                    "category", "battery"
                ),
