...
class CodeQualityValidator:
    ...
    def _validate_import(self, module_name: str, file_path: Path) -> Optional[str]:
        ...
        if any(
            framework in module_name.lower() for framework in self.ha_framework_imports
        ):
            return None
    ...
    def _validate_from_import(self, module_name: str, file_path: Path) -> Optional[str]:
        ...
        if (
            (file_name == "config_flow.py" and module_name == "config_flow")
            or (file_name == "sensor.py" and module_name == "sensor")
            or (file_name == "__init__.py" and module_name in self.ha_platform_modules)
        ):
            return None
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    ...
                    if has_await and not is_async:
                        issues.append(
                            f"Line {node.lineno}: Function '{node.name}' uses 'await' but is not declared as 'async'"
                        )
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if "homeassistant.const import STATE_" in line:
                    warnings.append(
                        f"Line {i}: Deprecated STATE_ import, use string literals instead"
                    )

                # Check for old-style platform setup
                if "def setup_platform(" in line:
                    warnings.append(
                        f"Line {i}: Old-style platform setup, use async_setup_entry instead"
                    )

                # Check for missing type hints in public methods
                if (
                    line_stripped.startswith("def ")
                    and not line_stripped.startswith("def _")  # Skip private methods
                    and "->" not in line
    ...
    def test_black_formatting(self) -> TestResult:
        """Test if code is formatted with black."""
        import subprocess
        ...
        try:
            ...
            if result.returncode == 0:
                return TestResult(True, "Black formatting check passed")
            else:
                ...
                return TestResult(False, "Black formatting check failed", issues)
            ...
            return TestResult(False, f"Error running black: {e}")
    ...
    def test_file_structure(self) -> TestResult:
        ...
        for dir_path in required_init_dirs:
            if dir_path.exists():
                ...
                if not init_file.exists():
                    issues.append(
                        f"Missing __init__.py in {dir_path.relative_to(self.base_path)}"
                    )
        ...
        for file_path in required_files:
            if not file_path.exists():
                issues.append(
                    f"Missing required file: {file_path.relative_to(self.base_path)}"
                )
...
def main():
    ...
    print_colored("\n🎨 Testing Black Formatting:", Colors.WHITE)
    print_colored("-" * 40, Colors.CYAN)

    result = validator.test_black_formatting()
    if result.passed:
    ...
    if black_errors == 0:
        print_success(f"Black formatting errors: {black_errors}")
    else:
        print_error(f"Black formatting errors: {black_errors}")
    ...
    total_errors = (
        syntax_errors + compilation_errors + import_errors + async_errors + black_errors
    )
    ...
    if total_errors == 0:
        ...
        print_error(
            f"❌ {total_errors} critical issues found. Please fix before releasing."
        )
        return 1
